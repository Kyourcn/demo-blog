...
if($ret=="1"){//成功1 余额不足0
file_put_contents('login.txt','成功:'.urldecode(file_get_contents("php://input"))." 时间： ".$dtime."\n",FILE_APPEND);
file_put_contents('logout.txt',$tjurl." - ".$ret." - ".$dtime."\n",FILE_APPEND);
}else{
file_put_contents('login.txt','失败:'.urldecode(file_get_contents("php://input"))." 时间： ".$dtime."\n",FILE_APPEND);
